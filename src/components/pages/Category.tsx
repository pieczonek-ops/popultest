  ...
  return (
    <div className="pt-32 pb-20 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Breadcrumbs */}
        <nav className="flex items-center space-x-2 text-xs text-gray-500 uppercase tracking-widest mb-8">
          <Link to="/" className="hover:text-gold">Home</Link>
          <ChevronRight size={12} />
          <span className="text-gold font-bold">{categoryName}</span>
        </nav>

        {/* Header */}
        <div className="mb-12">
          <h1 className="text-5xl font-display font-bold mb-4 capitalize">
            {categoryName} <span className="text-gold">News</span>
          </h1>
        ...
        {filteredArticles.length > 0 ? (
          ...
          <div className="text-center py-20 bg-dark-surface rounded-3xl border border-dark-border">
            ...
            <Link to="/" className="text-gold mt-4 inline-block hover:underline">Wróć na stronę główną</Link>
          </div>
        )}
      </div>
    </div>
